...
a.write = function()
	luci.sys.call("/usr/bin/zospusher/zospusher soc")
	luci.http.redirect(luci.dispatcher.build_url("admin","services","zospusher","advanced"))
end

if nixio.fs.access("/tmp/zospusher/soc_tmp") then
e=s:option(TextValue,"soc_tmp")
e.rows=2
e.readonly=true
e.cfgvalue = function()
	return luci.sys.exec("cat /tmp/zospusher/soc_tmp && rm -f /tmp/zospusher/soc_tmp")
end
end
...
a.description = translate("选项 1 选项 2 不会修改设置，并最多尝试 2 次。<br/>选项 3 会将设置项备份于 /usr/bin/zospusher/configbak 目录，并在失败后还原。<br/>【！！无法保证兼容性！！】不熟悉系统设置项，不会救砖请勿使用")
